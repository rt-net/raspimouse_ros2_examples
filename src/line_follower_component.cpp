...
void Follower::callback_light_sensors(
    const raspimouse_msgs::msg::LightSensors::SharedPtr msg) {
  // The order of the front distance sensors and the line following sensors are
  // not same
...
void Follower::callback_switches(
...
void Follower::publish_cmdvel_for_line_following(void) {
  ...
  bool detect_line = std::any_of(line_is_detected_by_sensor_.begin(),
                                 line_is_detected_by_sensor_.end(),
                                 [](bool detected) { return detected; });
  bool detect_field = std::any_of(line_is_detected_by_sensor_.begin(),
                                  line_is_detected_by_sensor_.end(),
                                  [](bool detected) { return !detected; });
...
void Follower::update_line_detection(void) {
  for (int sensor_i = 0; sensor_i < SENSOR_NUM; sensor_i++) {
    bool is_positive =
        present_sensor_values_[sensor_i] > line_thresholds_[sensor_i];
...
void Follower::beep_buzzer(const int freq,
...
CallbackReturn Follower::on_configure(const rclcpp_lifecycle::State &) {
  ...
  cmd_vel_timer_ =
      create_wall_timer(50ms, std::bind(&Follower::on_cmd_vel_timer, this));
  ...
  light_sensors_sub_ = create_subscription<raspimouse_msgs::msg::LightSensors>(
      "light_sensors", 1,
      std::bind(&Follower::callback_light_sensors, this, _1));
